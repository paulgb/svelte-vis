<script lang="ts">
  ...
  async function loadData() {
    ...
    let scale = (width - 2 * sideMargin - margin * (numCandidates - 1)) / numVotes;
      ...
      return d.allocations.map((e) => {
        ...
        if (round == 0) {
          ...
          labels.push({
            label: e.candidate,
            x: x + w / 2,
            y: heightPerRound * round + 10 + voteBlockHeight,
            anchor: 'end'
          })
        }
    ...
    let transferBlocks = data.flatMap((d, round) => {
      ...
      return d.transfers.map((t) => {
        ...
        return `
          M ${topX} ${top}
          L ${topX + width} ${top}
          C ${topX + width} ${midY} ${bottomX + width} ${midY} ${
          bottomX + width
        } ${bottom}
          L ${bottomX} ${bottom}
    ...
    let height =
      2 * labelMargin + (numRounds - 1) * heightPerRound + voteBlockHeight;
...
{:then data}
  <svg {width} height={data.height}>
    <g transform={`translate(0 ${labelMargin})`}>
      {#each data.labels as label}
        <text font-size="12" text-anchor={label.anchor} dominant-baseline="middle" transform={`translate(${label.x} ${label.y}) rotate(-90)`}>{label.label}</text>
      {/each}
